...
ModelSelectFinal module. The ModelSelection class is responsible for model 
selection and evaluation for both regression and classification tasks.
The script uses the unittest module to define and run various test scenarios
to ensure the correctness ModelSelection class methods.

Ensure that you have set up the necessary testing environment and data files
before running this script. Test file: 'project\\docs\\testing_Advertising.
...
- test_complete_data_incomplete: Tests handling of incomplete data.
- test_complete_data_digitalize: Tests digitalization of categorical data.
- test_preprocess_regression: Tests data preprocessing for regression tasks.
(Inplies classification)
...
- test_choose_model_regressor: Tests the selection of a regression model.
- test_choose_model_classifier: Tests the selection of a classification model.
- test_confirm_model_choice_continue: Tests model choice confirmation
with continuation.
- test_confirm_model_choice_exit: Tests model choice confirmation with exit.
...
- test_grid_model: Tests the grid_model method for hyperparameter tuning.
- test_calc_ideal_classification_model: Tests the calculation of the
ideal classification model.
- test_calc_ideal_regression_model: Tests the calculation of the ideal
regression model.
...
- test_data_report_plot_regression: Tests data reporting and plotting
for regression tasks.
- test_data_report_plot_classification: Tests data reporting and plotting
for classification tasks.

Usage:
Before running this script, ensure that you have set up the required testing
environment and data
files. You can run this script to perform unit tests on the ModelSelection class.

Notes:
There are no parameters for any test method because it is a test class
(and 'with'-staements are used instead of decorators when mocking.

No test mthod has a return because it is a test class.
...
class TestModelSelection(unittest.TestCase):
    ...
    """
    This class defines unit tests for the ModelSelection class in the ModelSelectFinal module.
    The tests cover various scenarios and methods in the ModelSelection class.
    """

    def setUp(self):
        '''
        Sets up the propers environment for testing each test method.
        ...
        self.reg_test_model : LinearRegression
            A test instance of a regression model.
        self.class_test_model : LogisticRegression
            A test instance of a classification model.

        """
        '''
        self.test_instance = ModelSelection()
        self.test_instance.data = pd.read_csv('docs\\testing_Advertising.csv') # 50 rows
    ...
    def tearDown(self) -> None:
        """
        Resets the testing environment to ensure independance between every test. This
        ensures independace between tests.
    ...
    def test_load_data_success(self) -> None:
        ...
        This method ensures that data has been loaded correctly and that x and y values are correctly defined.
        ...
        with patch('pandas.read_csv') as mock_read_csv:
            ...
            with patch('builtins.input', side_effect=['sales']): # UUser will 'choose' column 'sales' as target
        ...
        pd.testing.assert_frame_equal(
            ...
            self.test_instance.data.drop(columns=['sales'])) # Update with the correct column name
        pd.testing.assert_series_equal(
            self.test_instance.y,
            self.test_instance.data['sales']) # Update with the correct column name
    ...
    def test_initialize_model_reg(self) -> None:
        ...
        the model selection process is still set to regression.
        ...
        with patch('ModelSelectFinal.ModelSelection.choose_model') as mock_choose, \
            patch('ModelSelectFinal.ModelSelection.preprocess', side_effect=[[None,None,None,None]]) \
            as mock_pre, \
    ...
    def test_set_values(self):
        ...
        """
        # The first try an invalid than a valid choice('sales') to check execution
        # of both scenarios. This can be done in one mthod because they are in a loop.
        ...
        with patch('builtins.input', side_effect=user_tries):
            ...
            # pandas builtin assertion because otherwise 'the truth value of df is ambigous'
    ...
    def test_complete_data_drop_incomplete(self) -> None:
        """Test execution when data is incomplete and is dropped.
        ...
        with patch('builtins.input',return_value='Y'):
            ...
            with patch('ModelSelectFinal.ModelSelection.set_values') \
                as mock_set_values:
                mock_set_values.return_value = self.test_instance.data.dropna(), \
                                                self.test_instance.y
    ...
    def test_complete_data_incomplete_exit(self) -> None:#not working
    ...
    def test_complete_data_digitalize(self) -> None:
        ...
        self.assertTrue(
            self.test_instance.X.select_dtypes(include=['object']).columns.empty)

    def test_complete_data_no_digitalization_when_needed(self) -> None:#not working
        self.test_instance.X['feature1'] = ['option1', 'option2'] * 25
    ...
    def test_choose_model_r_to_c_conversion(self) -> None:
        """Test conversion if wrongly chosen.
    ...
    def test_choosee_model_conversion_denied(self) -> None:
        """Test exit execution when data conversion is denied.
    ...
    def test_confirm_model_choice_invalid_input_then_yes(self) -> None:
    ...
    def test_preprocess_regression(self) -> None:
        ...
        This method ensures proper execution of train_test_split, scaling and because it is
        regession also polynomial regression.
        """
        self.test_instance.regressor = True
    ...
    def test_save_model_invalid_valid(self) -> None:
        """Test axecution for both invalid and valid input. (Covering all outputs)
    ...
    def test_get_r2_score(self) -> None:
        """Test the proper fitting and predicting of a regression model,
        and ensureing correct return r2 score from passed model.
        """
    ...
    def test_calc_ideal_regression_model(self) -> None:
        ...
        This method ensures a report is made and later the call to confirm suggested
        model. It ensures the model that is suggested is a GridSearch-type meaning
        that they hyper-parameters have been tuned as the original estimator was converted
        it, ModelSelection.grid_model() rmse and r2 score are ensured to be called a
        correct amount of times.
        """
        self.test_instance.regressor = True
        # rmse & r2 need int output to be compared in the calculation. x5 amount of instances
    ...
    def test_calc_ideal_classification_model(self) -> None: # !!!!
        ...
        the confirmation of choice is called with a tuned model.
        """
    ...
    def test_classification_reports(self) -> None:
        ...
        value is being returned. I ensures that plots arent shown by default.
        """
    ...
    def test_classification_reports_show_plot(self) -> None:
        ...
        with patch('sklearn.metrics.ConfusionMatrixDisplay.from_estimator'), \
            ...
            self.test_instance.classification_reports(mock_model, show=True) # Set param True
    ...
    def test_regression_report(self) -> None:
        ...
        """Test that every metric is being printed for each model in a list.
    ...
    def test_data_report_plot_regression(self) -> None:
        ...
        self.test_instance.X = pd.concat([self.test_instance.X,self.test_instance.X],
                                        axis=1)
